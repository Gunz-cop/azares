function b(){const x=document.getElementById("dice-container"),g=document.getElementById("btn-spin"),c=document.getElementById("result-display"),C=document.querySelectorAll("#dice-count-selector .selector-btn"),I=document.querySelectorAll("#theme-selector .selector-btn"),h=document.getElementById("history-list"),A=document.getElementById("btn-clear"),m=document.getElementById("shake-permission-btn"),y=document.getElementById("toast-message");if(!x||!g||!c)return;let l=parseInt(localStorage.getItem("decidelo_dados_count"))||1,f=localStorage.getItem("decidelo_dados_theme")||"casino",v=!1,u=[],p=[{x:0,y:0},{x:0,y:0},{x:0,y:0}];const q={1:{x:0,y:0},2:{x:0,y:180},3:{x:0,y:270},4:{x:0,y:90},5:{x:270,y:0},6:{x:90,y:0}};let d=null;function H(){d||(d=new(window.AudioContext||window.webkitAudioContext)),d.state==="suspended"&&d.resume()}function R(t){try{H();const s=d.currentTime,i=4+t*3;for(let n=0;n<i;n++){const e=Math.random()*1.1,o=.8+Math.random()*.4;G(s+e,o)}for(let n=0;n<t;n++){const e=s+1.6+n*.1;N(e,.9+n*.08)}}catch{console.warn("Audio Context bloqueado o no soportado en este navegador.")}}function G(t,s){if(!d)return;const i=d.createOscillator(),n=d.createGain();i.type="triangle",i.frequency.setValueAtTime(950*s+(Math.random()-.5)*200,t),n.gain.setValueAtTime(.04,t),n.gain.exponentialRampToValueAtTime(1e-4,t+.025),i.connect(n),n.connect(d.destination),i.start(t),i.stop(t+.025);const e=d.createOscillator(),o=d.createGain();e.type="sine",e.frequency.setValueAtTime(130*s,t),o.gain.setValueAtTime(.07,t),o.gain.exponentialRampToValueAtTime(1e-4,t+.04),e.connect(o),o.connect(d.destination),e.start(t),e.stop(t+.04)}function N(t,s){if(!d)return;const i=d.createOscillator(),n=d.createGain();i.type="sine",i.frequency.setValueAtTime(110*s,t),i.frequency.exponentialRampToValueAtTime(50,t+.1),n.gain.setValueAtTime(.25,t),n.gain.exponentialRampToValueAtTime(1e-4,t+.12),i.connect(n),n.connect(d.destination),i.start(t),i.stop(t+.12);const e=d.createOscillator(),o=d.createGain();e.type="triangle",e.frequency.setValueAtTime(1100*s,t),o.gain.setValueAtTime(.08,t),o.gain.exponentialRampToValueAtTime(1e-4,t+.04),e.connect(o),o.connect(d.destination),e.start(t),e.stop(t+.04)}function k(t){y&&(y.textContent=t,y.classList.add("show"),setTimeout(()=>{y.classList.remove("show")},2500))}function O(){const t=localStorage.getItem("decidelo_dados_history");if(t)try{u=JSON.parse(t)}catch{u=[]}$()}function $(){if(h){if(h.innerHTML="",u.length===0){h.innerHTML='<span class="history-empty">Sin lanzamientos aún</span>';return}u.slice().reverse().forEach(t=>{const s=document.createElement("div");s.className="history-item-row";const i=document.createElement("span");i.className="history-sum",i.textContent=`Total: ${t.sum}`;const n=document.createElement("span");n.className="history-details",n.textContent=`[${t.details.join(", ")}]`,s.appendChild(i),s.appendChild(n),h.appendChild(s)})}}function Y(t){const i={sum:t.reduce((n,e)=>n+e,0),details:t};u.push(i),u.length>5&&u.shift(),localStorage.setItem("decidelo_dados_history",JSON.stringify(u)),$()}function w(){x.innerHTML="";for(let t=0;t<l;t++){const s=document.createElement("div");if(s.className="dice-scene",f==="dnd")s.innerHTML=`
          <div class="d20-wrapper" id="d20-wrapper-${t}">
            <svg viewBox="0 0 100 100" class="d20-svg">
              ...
              <polygon points="50,32 76,68 24,68" fill="url(#d20-grad-central-${t})" stroke="#0a0a0f" stroke-width="0.8" stroke-linejoin="round"/>
              <polygon points="50,2 91.6,26 50,32" fill="url(#d20-grad-side-${t})" stroke="#0a0a0f" stroke-width="0.8" stroke-linejoin="round"/>
              <polygon points="50,2 50,32 8.4,26" fill="url(#d20-grad-side-${t})" stroke="#0a0a0f" stroke-width="0.8" stroke-linejoin="round"/>
              <polygon points="8.4,26 50,32 24,68" fill="url(#d20-grad-side-${t})" stroke="#0a0a0f" stroke-width="0.8" stroke-linejoin="round"/>
              <polygon points="8.4,26 24,68 8.4,74" fill="url(#d20-grad-side-${t})" stroke="#0a0a0f" stroke-width="0.8" stroke-linejoin="round"/>
              <polygon points="24,68 8.4,74 50,98" fill="url(#d20-grad-side-${t})" stroke="#0a0a0f" stroke-width="0.8" stroke-linejoin="round"/>
              <polygon points="76,68 50,98 91.6,74" fill="url(#d20-grad-side-${t})" stroke="#0a0a0f" stroke-width="0.8" stroke-linejoin="round"/>
              <polygon points="76,68 91.6,74 91.6,26" fill="url(#d20-grad-side-${t})" stroke="#0a0a0f" stroke-width="0.8" stroke-linejoin="round"/>
              <polygon points="50,32 76,68 91.6,26" fill="url(#d20-grad-side-${t})" stroke="#0a0a0f" stroke-width="0.8" stroke-linejoin="round"/>
              <polygon points="24,68 76,68 50,98" fill="url(#d20-grad-side-${t})" stroke="#0a0a0f" stroke-width="0.8" stroke-linejoin="round"/>
              <text x="50" y="59" class="d20-number" id="d20-number-${t}" text-anchor="middle" dominant-baseline="central">20</text>
            </svg>
          </div>
        `;else{const i=f==="yugioh",n=i?"yugioh-face":"casino-face",e=i?"yugioh-dot":"casino-dot",o=i?`
